...
import { IUserProfile } from "../user/httpUser";
...
export class UserMatches{
    ...
    listInterestingBase=async() : Promise<Array<IInterest>>=>{
       ...
       const returnedData: Array<IInterest> = response.data.interests.map<IInterest>(el=>{
                  const interes: IInterest={interestText:el.interest_text.toLowerCase(),interest_id:el.id}
                  return interes;
                })
    ...
    likedUsers=async() : Promise<Array<IUserProfile>>=>{

        try {
       const response = await axiosMatches.get<Array<IUserProfile>>(
           `/likes/liked_users`,
       );
      
       console.log(response.data);
       
       const returnedData = response.data.map<IUserProfile>(el=>({...el,birth:new Date(Date.parse(el.date_of_birth))}))
    ...
    addFavoriteUsers=async(userId:number) : Promise<number>=>{

        try {
        
       const response = await axiosMatches.post<{}>(
           `/likes/add_to_favorites/${userId}`,
       );
    ...
    deleteFavoriteUsers=async(userId:number) : Promise<number>=>{

        try {
        
       const response = await axiosMatches.delete<{}>(
           `/likes/remove_from_favorites/${userId}`,
       );
