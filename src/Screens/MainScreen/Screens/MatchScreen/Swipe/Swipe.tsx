...
import { updateBunnerView } from "../../../../../store/reducers/userReducer";
...
const SwipeCards:FC<ISwipeCards>=({cards,getMatches,last,setUserModal})=>{
    ...
    const {isSubscription,bunnerView}=useAppSelector(state=>state.user)
    ...
    const goToMessage=()=>{
        if(!isSubscription){
            ...
            dispatch(updateBunnerView(true))
        }else{
            //TODO: открыть
        }
    }
    const [colo,setColo]=useState("red")
    const addUserToFavorite=(userId:number)=>{
       new UserMatches().addFavoriteUsers(userId);
    }
                ...
                return (
                    <>
                        <View style={[styles.card,{height:hHidth*0.73,width:wWidth-40,overflow:'hidden'}]}>
                            <ImagePreloader source={cardData.avatar_url}/> 
